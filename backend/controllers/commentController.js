		...
		try {
			...
			if (!comentario) {
				return response.json({
					message:
						"No se encontró el posteo para agregar un comentario",
				});
			}
...
const getCommentsByPostId = async (request, response) => {
	authenticateToken(request, response, async () => {
		try {
			...
			// Ordenar los comentarios por la fecha de creación
			const sortedComments = comments.sort(
				(a, b) => new Date(b.fecha) - new Date(a.fecha)
			);

			return response.status(200).json({
				cantidadComentarios: sortedComments.length,
				comentarios: sortedComments,
			});
	...
	authenticateToken(request, response, async () => {
		try {
			...
			if (!deletePost) {
				return response.json({
					message:
						"No se encontró el posteo para eliminar el comentario",
				});
			}
			return response
				.status(200)
